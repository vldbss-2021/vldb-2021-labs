...
func (s *testLogSuite) TestLogging(c *C) {
	...
	tlog := capnslog.NewPackageLogger("github.com/pingcap-incubator/tinykv/scheduler/pkg/logutil", "test")

	tlog.Info(fmt.Sprintf("[this message should not be sent to buf]"))
	...
	log.Warn(fmt.Sprintf("this message comes from logrus"))
	entry, err = s.buf.ReadString('\n')
	c.Assert(err, IsNil)
	c.Assert(entry, Matches, logPattern)

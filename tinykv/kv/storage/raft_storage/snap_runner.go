...
func (r *snapRunner) sendSnap(addr string, msg *raft_serverpb.RaftMessage) error {
	...
	log.Info(fmt.Sprintf("sent snapshot. regionID: %v, snapKey: %v, size: %v, duration: %s",
		snapKey.RegionID, snapKey, snap.TotalSize(), time.Since(start)))
	return nil
}
...
func (r *snapRunner) recvSnap(stream tinykvpb.TinyKv_SnapshotServer) (*raft_serverpb.RaftMessage, error) {
	...
	if snapshot.Exists() {
		log.Info(fmt.Sprintf("snapshot file already exists, skip receiving. snapKey: %v, file: %v", snapKey, snapshot.Path()))

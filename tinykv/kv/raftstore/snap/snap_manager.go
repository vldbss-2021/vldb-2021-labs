...
func (sm *SnapManager) Register(key SnapKey, entry SnapEntry) {
	log.Debug(fmt.Sprintf("register key:%s, entry:%d", key, entry))
	...
	if ok {
		for _, e := range entries {
			if e == entry {
				log.Warn(fmt.Sprintf("%s is registered more than 1 time", key))
				return
			}
		}
	}
...
func (sm *SnapManager) Deregister(key SnapKey, entry SnapEntry) {
	log.Debug(fmt.Sprintf("deregister key:%s, entry:%s", key, entry))
	sm.registryLock.Lock()
	defer sm.registryLock.Unlock()
	var handled bool
	entries, ok := sm.registry[key]
	...
	log.Warn(fmt.Sprintf("stale deregister key:%s, entry:%s", key, entry))
}
...
func (sm *SnapManager) DeleteSnapshot(key SnapKey, snapshot Snapshot, checkEntry bool) bool {
	...
	if checkEntry {
		if e, ok := sm.registry[key]; ok {
			if len(e) > 0 {
				log.Info(fmt.Sprintf("skip to delete %s since it's registered more than 1, registered entries %v",
					snapshot.Path(), e))
				return false
			}
		}
	} else if _, ok := sm.registry[key]; ok {
		log.Info(fmt.Sprintf("skip to delete %s since it's registered.", snapshot.Path()))
		return false
	}
	snapshot.Delete()
	return true

...
func NewSnap(dir string, key SnapKey, sizeTrack *int64, isSending, toBuild bool,
	...
	if util.FileExists(metaFile.Path) {
		...
		if err != nil {
			...
			log.Warn(fmt.Sprintf("failed to load existent snapshot meta when try to build %s: %v", s.Path(), err))
			if !retryDeleteSnapshot(deleter, key, s) {
				log.Warn(fmt.Sprintf("failed to delete snapshot %s because it's already registered elsewhere", s.Path()))
				return nil, err
			}
		}
	}
...
func (s *Snap) Build(dbSnap *badger.Txn, region *metapb.Region, snapData *rspb.RaftSnapshotData, stat *SnapStatistics, deleter SnapshotDeleter) error {
	if s.Exists() {
		...
		log.Error(fmt.Sprintf("[region %d] file %s is corrupted, will rebuild: %v", region.Id, s.Path(), err))
		if !retryDeleteSnapshot(deleter, s.key, s) {
			log.Error(fmt.Sprintf("[region %d] failed to delete corrupted snapshot %s because it's already registered elsewhere",
				region.Id, s.Path()))
			return err
		}
		err = s.initForBuilding()
	...
	log.Info(fmt.Sprintf("region %d scan snapshot %s, key count %d, size %d", region.Id, s.Path(), builder.kvCount, builder.size))
	err = s.saveCFFiles()
	if err != nil {
		return err
	}
...
func (s *Snap) Delete() {
	log.Debug(fmt.Sprintf("deleting %s", s.Path()))
	for _, cfFile := range s.CFFiles {
		if s.holdTmpFiles {
			_, err := util.DeleteFileIfExists(cfFile.TmpPath)
			if err != nil {
...
func (s *Snap) Save() error {
	log.Debug(fmt.Sprintf("saving to %s", s.MetaFile.Path))
	for _, cfFile := range s.CFFiles {
		if cfFile.Size == 0 {
			// skip empty cf file.
			continue
...
func (s *Snap) Apply(opts ApplyOptions) error {
	...
	for _, cfFile := range s.CFFiles {
		...
		if err != nil {
			log.Error(fmt.Sprintf("open ingest file %s failed: %s", cfFile.Path, err))
			return err
		}
	...
	if err != nil {
		log.Error(fmt.Sprintf("ingest sst failed (first %d files succeeded): %s", n, err))
		return err
	}
	log.Info(fmt.Sprintf("apply snapshot ingested %d tables", n))
	return nil
}

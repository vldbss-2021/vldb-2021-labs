...
func NewClient(pdAddrs []string, tag string) (Client, error) {
	...
	log.Info(fmt.Sprintf("[%s][scheduler] create scheduler client with endpoints %v", tag, urls))
	...
	log.Info(fmt.Sprintf("[%s][scheduler] init cluster id %v", tag, c.clusterID))
	c.wg.Add(2)
...
func (c *client) checkLeaderLoop() {
	...
	for {
		...
		if _, err := c.updateLeader(); err != nil {
			log.Error(fmt.Sprintf("[scheduler] failed updateLeader, err: %s", err))
		}
	}
}
...
func (c *client) switchLeader(addrs []string) error {
	...
	log.Info(fmt.Sprintf("[scheduler] switch leader, new-leader: %s, old-leader: %s", addr, oldLeader))
...
func (c *client) heartbeatStreamLoop() {
	...
	for {
		...
		select {
		case err := <-errCh:
			log.Warn(fmt.Sprintf("[%s][scheduler] heartbeat stream get error: %s ", c.tag, err))
			cancel()
			c.schedulerUpdateLeader()
			time.Sleep(retryInterval)
			wg.Wait()

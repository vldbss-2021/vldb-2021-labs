...
func (r *raftLogGCTaskHandler) gcRaftLog(raftDb *badger.DB, regionId, startIdx, endIdx uint64) (uint64, error) {
	...
	if firstIdx >= endIdx {
		log.Info(fmt.Sprintf("no need to gc, [regionId: %d]", regionId))
		return 0, nil
	}
...
func (r *raftLogGCTaskHandler) Handle(t worker.Task) {
	...
	if !ok {
		log.Error(fmt.Sprintf("unsupported worker.Task: %+v", t))
		return
	}
	log.Debug(fmt.Sprintf("execute gc log. [regionId: %d, endIndex: %d]", logGcTask.RegionID, logGcTask.EndIdx))
	collected, err := r.gcRaftLog(logGcTask.RaftEngine, logGcTask.RegionID, logGcTask.StartIdx, logGcTask.EndIdx)
	if err != nil {
		log.Error(fmt.Sprintf("failed to gc. [regionId: %d, collected: %d, err: %v]", logGcTask.RegionID, collected, err))
	} else {
		log.Debug(fmt.Sprintf("collected log entries. [regionId: %d, entryCount: %d]", logGcTask.RegionID, collected))
	}

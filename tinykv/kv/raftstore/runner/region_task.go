...
func (snapCtx *snapContext) handleGen(regionId uint64, notifier chan<- *eraftpb.Snapshot) {
	...
	if err != nil {
		log.Error(fmt.Sprintf("failed to generate snapshot!!!, [regionId: %d, err : %v]", regionId, err))
		notifier <- nil
	} else {
		notifier <- snap
	}
...
func (snapCtx *snapContext) applySnap(regionId uint64, startKey, endKey []byte, snapMeta *eraftpb.SnapshotMetadata) error {
	log.Info(fmt.Sprintf("begin apply snap data. [regionId: %d]", regionId))
	...
	log.Info(fmt.Sprintf("applying new data. [regionId: %d, timeTakes: %v]", regionId, time.Now().Sub(t)))
	return nil
}
...
func (snapCtx *snapContext) handleApply(regionId uint64, notifier chan<- bool, startKey, endKey []byte, snapMeta *eraftpb.SnapshotMetadata) {
	...
	if err != nil {
		notifier <- false
		log.Fatal(fmt.Sprintf("failed to apply snap!!!. err: %v", err))
	}
	notifier <- true
}
...
func (snapCtx *snapContext) cleanUpRange(regionId uint64, startKey, endKey []byte) {
	if err := engine_util.DeleteRange(snapCtx.engines.Kv, startKey, endKey); err != nil {
		log.Fatal(fmt.Sprintf("failed to delete data in range, [regionId: %d, startKey: %s, endKey: %s, err: %v]", regionId,
			hex.EncodeToString(startKey), hex.EncodeToString(endKey), err))
	} else {
		log.Info(fmt.Sprintf("succeed in deleting data in range. [regionId: %d, startKey: %s, endKey: %s]", regionId,
...
func doSnapshot(engines *engine_util.Engines, mgr *snap.SnapManager, regionId uint64) (*eraftpb.Snapshot, error) {
	log.Debug(fmt.Sprintf("begin to generate a snapshot. [regionId: %d]", regionId))

...
func (l *RaftLog) nextEntsSince(sinceIdx uint64) (ents []pb.Entry) {
	...
	if l.committed+1 > off {
		...
		if err != nil {
			log.Fatal(fmt.Sprintf("unexpected error when getting unapplied entries (%v)", err))
		}
		return ents
	}
	return nil
...
func (l *RaftLog) commitTo(tocommit uint64) {
	if l.committed < tocommit {
		if l.LastIndex() < tocommit {
			log.Fatal(fmt.Sprintf("tocommit(%d) is out of range [lastIndex(%d)]. Was the raft log corrupted, truncated, or lost?", tocommit, l.LastIndex()))
		}
...
func (l *RaftLog) restore(s pb.Snapshot) {
	log.Info(fmt.Sprintf("log [%+v] starts to restore snapshot [index: %d, term: %d]", l, s.Metadata.Index, s.Metadata.Term))
...
func (l *RaftLog) mustCheckOutOfBounds(lo, hi uint64) error {
	if lo > hi {
		log.Fatal(fmt.Sprintf("invalid slice %d > %d", lo, hi))
	}
	fi := l.firstIndex()
	if lo < fi {
		return ErrCompacted
	...
	if hi > l.LastIndex()+1 {
		log.Fatal(fmt.Sprintf("slice[%d,%d) out of bound [%d,%d]", lo, hi, fi, l.LastIndex()))
	}
	return nil
}
...
func (l *RaftLog) zeroTermOnRangeErr(t uint64, err error) uint64 {
	...
	log.Fatal(fmt.Sprintf("unexpected error (%v)", err))
	return 0
}
